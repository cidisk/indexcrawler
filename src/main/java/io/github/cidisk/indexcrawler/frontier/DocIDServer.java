...
package io.github.cidisk.indexcrawler.frontier;
...
public class DocIDServer extends Configurable {
  ...
  public DocIDServer(Environment env, CrawlConfig config,URLType type) throws DatabaseException {
    super(config);
    DatabaseConfig dbConfig = new DatabaseConfig();
    dbConfig.setAllowCreate(true);
    if(type == URLType.INDEX){
    	 dbConfig.setTransactional(config.isResumableIndexCrawling());
    	    dbConfig.setDeferredWrite(!config.isResumableIndexCrawling());
    	    docIDsDB = env.openDatabase(null, "DocIDs", dbConfig);
    	    if (config.isResumableIndexCrawling()) {
    	      int docCount = getDocCount();
    	      if (docCount > 0) {
    	        logger.info("Loaded {} index URLs that had been detected in previous crawl.", docCount);
    	        lastDocID = docCount;
    	      }
    	    } else {
    	      lastDocID = 0;
    	    }
    }else if(type == URLType.INFO){
    	dbConfig.setTransactional(config.isResumableInformationCrawling());
	    dbConfig.setDeferredWrite(!config.isResumableInformationCrawling());
	    docIDsDB = env.openDatabase(null, "InformationPagesDB", dbConfig);
	    if (config.isResumableInformationCrawling()) {
	      int docCount = getDocCount();
	      if (docCount > 0) {
	        logger.info("Loaded {} information URLs that had been detected in previous crawl.", docCount);
	        lastDocID = docCount;
	      }
	    } else {
	      lastDocID = 0;
