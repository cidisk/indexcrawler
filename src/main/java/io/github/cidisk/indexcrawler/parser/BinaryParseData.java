...
public class BinaryParseData implements ParseData {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(BinaryParseData.class);
	...
	public void setBinaryContent(byte[] data) {
		...
		try {
			TransformerHandler handler = getTransformerHandler(outputStream,
					DEFAULT_OUTPUT_FORMAT, DEFAULT_ENCODING);
			AUTO_DETECT_PARSER.parse(inputStream, handler, new Metadata(),
					context);

			// Hacking the following line to remove Tika's inserted DocType
			String htmlContent = new String(outputStream.toByteArray(),
					DEFAULT_ENCODING).replace("http://www.w3.org/1999/xhtml",
					"");
			setHtml(htmlContent);
	...
	private static TransformerHandler getTransformerHandler(OutputStream out,
		...
		TransformerHandler transformerHandler = SAX_TRANSFORMER_FACTORY
